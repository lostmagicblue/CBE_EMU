...
> 状态：协议回归已经完成。服务端“任务使者”自 2026-07-18 起不再加入
> 蓬莱-铸剑谷或其他场景的 `27/11` NPC 目录；以下内容保留为历史协议证据。
...
- `net_handle_task_response_dispatch(0x0104726C)` 的 case 4 才是正常提交结果；成功值为 `result=1`，并读取经验、等级、活力、奖励和 `taskdes`。case 16 调用 `HandleTaskCompleteResult(0x01038E6E)`，其成功提示是硬编码“重置成功!”，不能用于任务提交。

IDA 实例按 `binary_name=江湖OL.CBE` 选择，未固定 instance id。

## 服务端契约

- 历史测试场景 NPC：actor `20022`，名称“任务使者”，模型 `n_man1.actor`，坐标
  `(300,125)`；当前版本已移除该场景种子。
...
- `6/6`：保存并回送任务状态。
- `6/4 -> 6/4`：仅允许状态 `2` 的任务提交，成功后保存状态 `3`；响应以 `result=1` 下发角色当前经验、等级、活力、奖励信息和提交成功正文。
...
- 完成交互响应同包包含 `26/1 + 6/6 state=2`；提交响应为 `6/4 {result=1,...}`，提交后任务子集不再包含 task id。
...
同轮系统性核对还修正了后续两段契约：`6/10 info` 改为 `SendTaskHallReq` 实际读取的 WT string；最终提交由客户端上行 `6/4`，服务端也下发任务分派 case 4 所需的 `6/4 {result=1,...}` 完成结果。

- `make -j2` 通过。
- 独立 service clientId 验证 NPC 选项序列中任务名后为 `action=4 + taskid=900001`，响应长度 `123`。
- `6/10` 详情响应长度 `68`，对象为 `6/10` 且包含纯中文正文；`6/11` 接取响应长度 `133`。
- 完成交互返回 `26/1 + 6/6`；上行 `6/4` 得到下行 `6/4 {result=1,...}`。
...
`0x01047D56` 创建 `6/4 {taskid}`；任务响应分派 `0x0104726C` 的 case 4
读取 `result=1`、经验/等级/活力、奖励和 `taskdes`，随后从活动任务列表删除任务并
重算场景 NPC 提示标识。

任务处理器现只额外接受这一种精确的前置组合，并保持请求语义顺序返回两个对象：

- `1/25/5 {result=4}`：结束等待提示；
- `1/6/4 {result=1,energy,energymax,exp,level,lastexp,curexp,persentexp,seqnum,iteminfo,awardinfo,taskdes}`：提交完成结果。

其他任务请求仍必须以 `1/6/*` 开头；`6/11 + 空 25/5` 的既有接取组合保持不变，
避免前置等待对象规则误吞其他复合包。

- `make -j2` 通过。
- 回归严格发送真实 29 字节请求，收到对象顺序
  `25/5 + 6/4` 的响应；服务日志显示
  `request_info_prefix=1 response_objects=2 result=1`。
...
## 2026-07-18 任务瞬移进度条修正

真实客户端在活动任务详情点击“瞬移”后发送：

```text
WT 1/16/5 { taskid:u32 }
总长 24，object payload 长 15
```

旧服务没有任务瞬移 handler，运行时记录
`unhandled wt=16/5 len=24 first=1/16/5:15` 后在未处理包断言处退出；客户端的
等待状态因此永远得不到业务响应。该请求不是传送石 `16/1..4` 协议，不能复用
传送石确认包。

IDA 按 `binary_name=江湖OL.CBE` 动态选择实例。客户端证据如下：

- `SendTaskTransportReq(0x01047E9A)` 首次以 `taskid` 发送 `16/5`；目的地列表
  选择后，同一函数改以 `transid` 再发 `16/5`。
- `task_handle_destinfo_response(0x01047F0A)` 的 `16/5` 分支读取
  `destnum + destinfo`。`destinfo` 每行依次为 `u32 transid + string label`，
  成功解析后进入目的地列表状态；即使 `destnum=0` 也会走客户端原生的无目的地
  提示并清除等待状态。
- 同一解析器的 `16/6` 分支读取
  `text/destscene/scenename/transid`，然后由客户端自身进入同场景定位或跨场景
  传送流程，并在分支尾清除网络等待标志。同场景分支还会把原响应传给
  `HandleItemUseConfirm(0x010190A8)`；该函数继续读取 `result/value`，其中
  `value` 是确认框显示并在确认后消费的传送石数量。

服务端新增窄处理器 `builtin-task-transport`：

- 仅接受单对象 `1/16/5`，且必须恰有 `taskid` 或 `transid` 之一。
- `taskid` 必须属于当前角色状态 `1/2` 的活动任务；返回
  `1/16/5 {destnum,destinfo}`。
- `transid` 返回
  `1/16/6 {text,destscene,scenename,transid,result=0,value=1}`，与已验证的
  传送石确认语义一致，避免同场景目的地依赖缺字段的默认读取结果。
- 测试任务定位到 `00蓬莱仙岛_02.sce` 的任务使者。
- 正式任务优先用 `task.dsh` 目标文本匹配 `sMap.dsh` 地图别名；纯对话任务再用
  交付者名称扫描正式 SCE，包括旧版 `NN_场景NN.sce` 与现行 SMap 场景键之间的
  对应关系。运行时不读取 `tmp/all_sce_export`。
- 无法解析的活动任务返回 `destnum=0`，不再让服务断言或遗留进度条。

协议回归结果：

- 测试任务 `900001`：列表响应长 `55`，选择响应长 `128`。
- 正式任务 `1`：从任务目标/交付者解析到
  `c00蓬莱仙岛_01.sce / 蓬莱-铜雀台`；列表响应长 `55`，选择响应长 `129`。
- 两轮均得到 `source=builtin-task-transport`、普通 event `7`、stderr `0`；正式
  任务回归使用的临时 MySQL 行已删除。
- 构建命令 `make -j2` 通过。

后台回归还修正了 Windows 无控制台启动场景：只有标准输入确实是控制台时才
轮询 `_kbhit/_getch`，并提供 `CBE_MOCK_SERVICE_DISABLE_CONSOLE=1` 显式关闭
控制台命令轮询，防止后台服务端口已经监听、主循环却阻塞在控制台输入。
