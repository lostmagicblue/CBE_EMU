...
| 接受方进入交易 | `21/4`（`21/3` 直回） | `result:u8=1`, `name` | `0x01011132`；确认回调本身不会切换界面，必须解析 subtype 4 后调用 `trade_switch_ui_mode(1)` |
| 发起方结果 | `21/4` | `result:u8`, `name` | `0x01011132`；同意时进入客户端交易界面，拒绝时显示提示 |
| 本方提交报价 | `21/5` | `num:u8`, `trademoney:u32`, `iteminfo` | `SendTradeSellReq(0x01022D4E)`；`iteminfo` 每行是背包 `seq` 与报价数量 |
| 对方报价刷新 | `21/6` | `num:u8`, `trademoney:u32`, `iteminfo` | `0x01025AE6`；按完整物品行刷新交易面板 |
| 最终确认/取消 | `21/7` | `result:u8` | `0x01022E24` 发送；`0x01027726` 解析等待、取消和报价失效 |
| 交易结算 | `21/8` | `result:u8`, 成功时另有 `num`, `money`, `iteminfo` | `0x01027726 -> BuildShopBuyList(0x010259B6)`；删除本方报价并加入对方物品 |

好友列表的 `21/1.id` 与同一列表的装备查看、组队邀请一样，是持久角色 ID，
目标不一定处在发起方当前场景。服务端现在先按当前场景 actor id 解析；失败后只允许
匹配当前角色已确认好友关系的持久 role id；只有该好友存在有效在线 session 时才排队投递邀请。
这修复了 `guest00023` 从好友列表向 `guest00024/10024` 发起交易时的
`unhandled wt=21/1 len=20`，同时没有把 `21/1` 扩大为可请求任意在线角色。

接受方同意后，服务端在 `21/3` 的直接响应中返回
`21/4 { result=1, name=<发起方> }`，并把镜像的 `21/4` 排队给发起方；两端因此都通过
客户端原生 subtype-4 分支进入交易界面。拒绝时接受方只收到空 WT 确认，发起方在下一次
场景同步轮询收到 `21/4 { result=2, name=<拒绝方> }`，避免拒绝者看到方向相反的提示。

双账号回归使用互为好友但位于 `c00蓬莱仙岛_01`、`c00蓬莱仙岛_03` 的
`guest00023/10023` 与 `guest00024/10024`，确认目标解析日志为 `scope=friend queued=1`；
接受链路为 `21/1 -> poll 21/2 -> 21/3 -> direct 21/4 + poll 21/4`，拒绝链路为
`21/1 -> poll 21/2 -> 21/3 -> empty WT + poll 21/4(result=2)`。运行记录：
`tmp/mock-service-19090.20260718-1333.stdout.log`，stderr 为 0。

交易会话由两个在线 client id 绑定，双方各自保存一份最新报价。合法的 `21/5`
会重新校验金币余额、背包 `seq`、数量和重复行，清除旧确认状态，并把镜像 `21/6`
放入对端正常场景同步轮询；因此修改报价后必须重新确认。第一次 `21/7 result=1`
只返回客户端原生的“等待对方确认交易”，第二次确认才执行结算。主动取消、报价缺失、
角色断线都会终止会话并向仍在线的一方投递 `21/7 result=2`，不会残留占用中的交易槽位。

结算先在两份角色状态副本上完成物品扣除、金币互换、目标背包容量和数值溢出检查，
...
`7/21` 的发送字段也是 `opengold`；它们属于经验/挂机设置，不参与交易报价。

2026-07-18 双客户端回归覆盖物品+金币交换、主动取消和交易中断线。物品场景中甲方
从 3 个 `itemId=800` 报价 2 个，乙方报价 5 金币；结算后 MySQL 为甲方
`money=105/item_count=1`、乙方 `money=95/item_count=2`，双方 `next_backpack_seq=2`。
第一次确认日志为 `21/7 result=1 commit=0`，第二次为 `21/8 result=1 commit=1`；
取消与断线均由对端收到 `21/7 result=2`。运行记录：
`tmp/mock-service-19194.20260718-145953.stdout.log`，stderr 为 0。
...
`net_handle_login_or_name_result(0x0101258A)` 补全了切磋邀请的主 CBE
协议：

- 发起端发送 `1/4/14 { id }`；成功响应为
  `1/4/14 { result=1 }`。`result=2..7` 分别进入客户端已有的战斗中、
  目标状态、目标不存在、超出范围和本机状态错误提示分支。
- 目标端通过场景同步轮询收到 `1/4/15 { id, name }`。客户端保存 `id`，
  并打开“`<name>请求和你切磋,确定切磋?`”原生确认框。
- `SendNPCDialogResult2(0x010124EE)` 回发
  `1/4/16 { id, result }`，其中 `1=同意`、`2=拒绝`；同意路径在
  `HandleSceneNPCInput(0x01012528)` 中还会追加 `1/4/9 { id }`。
- 发起端通过轮询收到 `1/4/17 { result, id, name }`。客户端对
  `result=2` 显示“`<name>不同意和你切磋!`”；`result=1` 是静默确认。

服务端为切磋新增独立的 `spar-invite/spar-result` 社交通知，并使用接收方
视角的远端 actor id，避免旧角色 id 冲突。邀请只会在发起者仍在线且双方仍在
同一可见场景时下发；确认框未处理期间不会叠加其他模态邀请。`4/16` 和可选的
同包/后续 `4/9` 都有窄处理器，因此点击同意或拒绝不会落入未处理包断言。

运行回归使用 `guest00023/10023` 与 `guest00024/10024`（共同位于
`07泰山_01.sce`），验证了：

- `4/14 -> 4/15` 邀请投递；
- `4/16 result=2 -> 4/17 result=2` 拒绝回执；
- `4/16 result=1 + 4/9 -> 4/17 result=1` 同意回执；
- 全流程没有未处理 WT。
...
观察端 A：左=B，右=A
观察端 B：左=A，右=B
side=1, left_count=1, right_count=1
```

在这个布局中，`sub_2B26(0x2B26)` / `CalcTargetSideIndex(0x6CE8)` 的稳定映射为：

```text
本机行动：actor wire 0 -> target wire 1
...
未处理 WT。最终含技能与死亡回归的日志为
`tmp/mock-service-19090.20260720-003829.stdout.log`，对应 stderr 长度为 0。

### 其余邀请与请求

入帮邀请发送器仍会在发起端立即显示“已发送”提示，服务端目前仅返回空 WT
传输确认；它的目标端入站通知与接受/拒绝协议尚未根据客户端证据实现。
