...
- `sMap.dsh` 第 55 行：`c04临安府_09.sce`，别名“临安-北宣门”；
- 当前 `c04临安府_09.sce` 解压后为 414 字节，包含五个传送口、零个满足 `.actor + .xse + 名称 + 坐标` 的交互 NPC，因此三人必须由服务端目录下发；
- 历史客户端场景资源将王朝绑定到 `n_solider2.actor / 04临安王朝.xse`，将马汉绑定到 `n_solider1.actor / 04临安马汉.xse`；旧 `c04临安府_10.sce` 模板虽然把胡斐绑定到 `n_blacksmith.actor / 04临安胡斐.xse`，但该持锤铁匠模型与当前北宣门实际人物外观不符；
...
- `20090` 王朝：`(172,132)`；
- `20091` 马汉：`(228,132)`；
- `20092` 胡斐：`n_man1.actor`，`(304,304)`。

北宣门当前地图为 400×400。王朝、马汉位于通往 `06野猪林_04.sce` 的北门触发区下方两侧；胡斐位于右下商铺前。三个点都落在当前 map 的零标志可行走格内。胡斐改用 `n_man1.actor`：本地场景资源把该青年男性模型用于段誉、小鱼儿、韦小宝、林冲等人物，比持锤的 `n_blacksmith.actor` 更符合胡斐的显示身份。胡斐脚本仍保留客户端真实的 `04临安胡斐.xse`。原服务端坐标和模型目录尚未找到，因此坐标按当前地图碰撞与门口布局确定，模型选择属于基于现有客户端资源用例的校正，而非声称恢复了原服唯一值。运行时仍不读取 `tmp/all_sce_export`，该历史导出只作为人工证据。

画面中看似新出现的房屋不是 `27/11` 动态 NPC 行带出的对象。未下发任何动态 NPC 的 `04临安府_09.map` 原始渲染已经包含相同的中央及四角建筑；`n_blacksmith.actor` 清单只有一张单帧、单部件、约 30×38 像素的角色图，也不含建筑资源。服务端没有修改或附加 map/SCE 建筑层。
...
- NPC 一次性生命周期比较统一忽略 c 前缀场景键的可选 `.sce` 后缀。数据库角色场景可能保存 `c04临安府_09.sce`，而 ready session 使用 `c04临安府_09`；严格 `strcmp` 会在 poll 前错误清除 pending，使首登北宣门不下发目录。
...
- `make -j2`：铸剑谷与临安-北宣门动态目录改动均已通过构建。
...
- 使用原本就停留在北宣门的 `guest00023` 做首登实测：服务端输出 `source=service-dynamic actors=3 rows=3 dynamic=3 npcinfo_len=262`，随后由 `startup-scene-sync-poll` 下发 `npcnum=3`，组合 poll 为 357 字节并通过普通 event 7 到达客户端。
- 胡斐模型校正后的复测目录为 `source=service-dynamic actors=3 rows=3 dynamic=3 npcinfo_len=250`，组合 poll 为 345 字节；客户端场景节点表仍出现 `20090 pos=(172,132)`、`20091 pos=(228,132)`、`20092 pos=(304,304)`，三者均为 `kind=1 prompt=21 active=1`。画面显示王朝、马汉为两种守卫模型，胡斐为 `n_man1.actor` 蓝衣青年男性模型，无地址异常或断言。
- 烟雾测试结束后已将 `guest00023` 恢复到测试前的 `scene=c04临安府_09.sce, pos=(161,165), hp=17/120`，最终服务重启后重新从恢复值加载。
