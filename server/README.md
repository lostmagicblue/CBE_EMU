...
The service also needs the real task/scene resources under
`web/fs/JHOnlineData`. This directory is not generated by `make`; copy it with
the server deployment. When starting outside the project directory, point the
service at the project root (the directory containing `web`) with either:

```bash
CBE_RESOURCE_ROOT=/mnt/vdb/JHOnline ./bin/jh-online-server
