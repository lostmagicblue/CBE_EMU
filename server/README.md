...
- mock logic is exposed through a TCP service (Linux server default bind is `0.0.0.0`)
...
The Linux game service and web admin listeners default to `0.0.0.0:19090` and
`0.0.0.0:19091`. Override them with `CBE_MOCK_SERVICE_BIND` /
`CBE_MOCK_ADMIN_BIND`, or `--mock-service-bind=` / `--mock-admin-bind=`. Use the
server's real IP address, rather than `0.0.0.0`, when opening the admin page.

The service also needs the real task/scene resources from `JHOnlineData`. This
directory is not generated by `make`; copy it with the server deployment.
`CBE_RESOURCE_ROOT` may point either directly to the `JHOnlineData` directory
or to a project root containing `web/fs/JHOnlineData` or `bin/JHOnlineData`:

```bash
CBE_RESOURCE_ROOT=/mnt/vdb/JHOnline/bin/JHOnlineData ./bin/jh-online-server
